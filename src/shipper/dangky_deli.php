...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $loaixe = $conn->real_escape_string($_POST['loaixe']); 
    $khuvuchoatdong = $conn->real_escape_string($_POST['khuvuchoatdong']);

    if ($matkhau !== $xacnhan_matkhau) {
        $message = "Mật khẩu và Xác nhận mật khẩu không khớp. Vui lòng kiểm tra lại.";
        $message_type = "error";
    } else {
        ...
        if ($anh_daidien === null || $anh_cccd_truoc === null || $anh_cccd_sau === null || $anh_banglaixe === null) {
            ...
            if ($conn->query($sql_parent) === TRUE) {
                ...
                $sql_child = "INSERT INTO $tableName (
                                    ID_NguoiDung, GioiTinh, NamSinh, BienSoXe, LoaiXe, KhuVucHoatDong, 
                                    Anh_CCCD_Truoc, Anh_CCCD_Sau, Anh_BangLaiXe
                                ) VALUES (
                                    $ID_NguoiDung_moi, '$gioitinh', $namsinh, '$biensoxe', '$loaixe', '$khuvuchoatdong', 
                                    '$anh_cccd_truoc', '$anh_cccd_sau', '$anh_banglaixe'
                                )";
...
<head>
    ...
    <title>Đăng Ký Ứng Viên Giao Hàng</title>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', (event) => {
            ...
            form.addEventListener('submit', function(e) {
                ...
                const xacnhan_matkhau = document.getElementById('xacnhan_matkhau').value;
                ...
                if (matkhau !== xacnhan_matkhau) {
                    ...
                    errorDiv.id = 'message-box';
                    errorDiv.className = 'p-4 mb-4 text-sm rounded-lg bg-red-100 text-red-700 border border-red-400';
                    errorDiv.setAttribute('role', 'alert');
                    errorDiv.textContent = 'Mật khẩu không khớp. Vui lòng kiểm tra lại.';
...
<body class="flex items-center justify-center min-h-screen p-4">
    ...
    <div class="w-full max-w-2xl bg-white shadow-2xl rounded-xl p-8 md:p-10 border border-gray-100 my-8">
        ...
        <form method="POST" action="" enctype="multipart/form-data"> 
            ...
            <fieldset class="mb-8 border border-gray-200 rounded-xl p-6 shadow-sm">
                <legend class="text-lg font-bold text-blue-700 px-2 ml-4">Thông tin Cơ bản </legend>
                ...
                <div class="mb-4">
                    <label for="xacnhan_matkhau" class="block text-sm font-medium text-gray-700 mb-1">Xác nhận mật khẩu (*)</label>
                    <input type="password" id="xacnhan_matkhau" name="xacnhan_matkhau" required minlength="6" 
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500">
                </div>
                </fieldset>
            
            <fieldset class="mb-8 border border-gray-200 rounded-xl p-6 shadow-sm">
                <legend class="text-lg font-bold text-blue-700 px-2 ml-4">Thông tin Phương tiện & Hoạt động 🛵</legend>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div class="mb-4">
                        <label for="loaixe" class="block text-sm font-medium text-gray-700 mb-1">Loại xe (*)</label>
                        <select id="loaixe" name="loaixe" required
                            ...
                            <option value="Xe máy">Xe máy (Dưới 175cc)</option>
                            <option value="Xe máy (Lớn)">Xe máy (Trên 175cc)</option>
            ...
            <fieldset class="mb-8 border border-gray-200 rounded-xl p-6 shadow-sm">
                <legend class="text-lg font-bold text-blue-700 px-2 ml-4">Tài liệu Xác minh 📄</legend>
            ...
            <button type="submit" class="w-full flex justify-center items-center py-3 px-4 border border-transparent rounded-xl shadow-lg text-lg font-semibold text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-4 focus:ring-blue-500 focus:ring-opacity-50 transition duration-150 ease-in-out">
                HOÀN TẤT ĐĂNG KÝ
            </button>
            <p class="text-center text-sm text-gray-600 mt-4">Bạn đã có tài khoản? <a href="login.php" class="text-blue-600 hover:text-blue-800 font-medium hover:underline">Đăng nhập</a></p>
        </form>
    </div>
</body>
</html>